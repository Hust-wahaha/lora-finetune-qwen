...
DATASET_PATH = ROOT / 'data' / 'final' / 'pilot.jsonl'
RUNS = ROOT / 'runs'
...
def main() -> None:
    ...
    with (run_dir / 'metrics' / 'run_config.json').open('w', encoding='utf-8') as f:
        json.dump({'model': MODEL_ID, 'dataset': str(DATASET_PATH), 'run_dir': str(run_dir)}, f, ensure_ascii=False, indent=2)
    ...
    train_dataset, val_dataset = load_dataset(str(DATASET_PATH), split_dataset_ratio=0.5, seed=42)
